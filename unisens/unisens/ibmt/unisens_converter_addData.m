...
% this function can be used to add data to an existing unisens dataset from
% data structure --> it's en extension to unisens_converter.m
%
...
function unisens_converter_addData(pathname, datasetname, data)
...
%check if the defined unisens record already exists
createNewDataSet=0;
if(isdir([pathname '\' datasetname]) & exist([pathname '\' datasetname '\' 'unisens.xml'],'file'))
    disp('writing additional information')
    ...
    createNewDataSet=1; %--> function could be modified to write data if the data set is not available, just remove return command
    disp('No dataset available aborting operation.');
    return;
end
